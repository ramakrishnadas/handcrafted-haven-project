...
const CreateUser = UserSchema.omit({ id: true, name:true, profile_image:true, user_story: true});
...
const UpdateUser = UserSchema.omit({ id: true, firstName:true, lastName:true});

export async function updateUser(id: string, prevState: State2, formData: FormData) {
  ...
  const { name, email, password, profile_image, user_story } = validatedFields.data;
...
export async function updateProduct(id: string, prevState: State3, formData: FormData) {
  ...
  const { product_name, category, price, image_url, description, stock, thumbnail_image_url } = validatedFields.data;
